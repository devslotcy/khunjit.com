...
export default function PatientMessages() {
  ...
  // Auto-select conversation from localStorage if available
  useEffect(() => {
    const storedConvId = localStorage.getItem('selectedConversationId');
    if (storedConvId && conversations?.some(c => c.id === storedConvId)) {
      setSelectedConversation(storedConvId);
      localStorage.removeItem('selectedConversationId'); // Clear after use
    }
  }, [conversations]);
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            <p className="font-medium text-sm sm:text-base truncate flex items-center gap-2">
                              {conv.psychologist?.fullName || "Psikolog"}
                              {conv.psychologistId === 'system' && (
              ...
              <>
                ...
                <div className="p-3 sm:p-4 border-b border-card-border flex items-center gap-3 bg-card">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-sm sm:text-base truncate">{selectedConv.psychologist?.fullName || "Psikolog"}</p>
                    <p className="text-xs text-muted-foreground">{selectedConv.psychologist?.title || "Klinik Psikolog"}</p>
                  </div>
                </div>
                ...
                {messagesLoading ? (
                  ...
                  <MessageThread
                    ...
                    otherUserId={selectedConv.psychologistId}
